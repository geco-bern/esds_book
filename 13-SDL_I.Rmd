...
![Features extraction from a CNN (convolutional neural network).](./figures/Chollet_Fig5.4.png)
![Instance classification.](./figures/Chollet_Fig5.2.png)

### Building Blocks of a CNN
You probably already know this or at least guessed it but CNN stands for convolutional neural network. 
...
You can think of a convolution as sliding a window, also called kernel or filter, over each pixel of an image and computing a dot product between the filter's values and the image's values that the window (or filter) is covering. This operation produces one output value for every location on the image over which we slide the filter. 

![](https://raw.githubusercontent.com/vdumoulin/conv_arithmetic/master/gif/no_padding_no_strides.gif)

Here, in blue is the input image, grey (or a darker blue) is the 3-by-3 filter that we are convolving the input image with and green is the output image.


After the convolutional layer usually a max-pooling layer is applied which downsamples the feature map. Through this only certain aspects of information in our original image are extracted.
...
cat('total training dog images:', length(list.files(file.path(train_dir,'dogs'))),"\n")

#validation set
cat('total validation cat images:', length(list.files(file.path(validation_dir,'cats'))),"\n")
cat('total validation dog images:', length(list.files(file.path(validation_dir,'dogs'))),"\n")

#test set
cat('total test cat images:', length(list.files(file.path(test_dir,'cats'))),"\n")
cat('total test dog images:', length(list.files(file.path(test_dir,'dogs'))),"\n")
```
...
#We have already run it and we saved the results because it takes more than 30 minutes for 20 epochs. But you can run in if you are curious.
...
history = readRDS('./saved_models/models_history.rds')
...
* `rescale`: You've seen this in our first use of 'ImageDataGenerator'. This simply rescales the RGB values to be between 0 and 1. <br> 
<br> 
* `rotation_range`: is the range in degrees of permitted image rotations.  <br> 
<br> 
* `width_shift_range`: shifts the image to the left or to the right by the fraction of the total width specified (i.e., 0.1 in this example).<br> 
<br> 
* `height_shift_range`: shifts the image up or down by the fraction of the total width specified (i.e., 0.1 in this example).<br> 
<br> 
* `shear_range`: distorts the image along an axis, in effect approximating a change in the position of the photographer. <br> 
<br> 
* `zoom_range`: zooms in and out of the image. <br> 
<br> 
* `horizontal_flip`: randomly flips the image along the horizontal axis.<br> 
<br> 
...
To get your own feel for what this function does, and to enjoy some funny cat pictures, execute the code cell below with different cats by just changing the index to the list _fnames_ on line 5.
...
#plot them
fig_size(20,8)
p_base + p_L2 + p_dropout + p_data_augment+plot_layout(nrow = 1)
```
...
```{r}
#load the model
model = load_model_hdf5('./saved_models/final_model.h5')
```
...
Great, we have a well performing model.

Now let's interpret what the CNN learns.
...
<div style="background-color:rgba(0, 200, 0, 0.5);border-radius: 7px; padding:10px">
    <b>Checkpoint</b><br/>
Go and choose pic 50 from dogs. What do you observe?

### Answer
Amazing result!!! Dog leash is activated!!! That can be a very powerful feature for our classifier.
