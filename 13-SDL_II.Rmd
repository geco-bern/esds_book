...
- Develop critical judgment, assess the relevance of a method for a particular modelling objective.
...
```{r eddytower, out.width="25%", echo=FALSE, fig.cap="Close-up of measuring device for $CO_2$ and other environmental parameters that is attached to eddy covariance towers. Image taken from [Wikipedia](https://upload.wikimedia.org/wikipedia/commons/5/52/Eddy_Covariance_IRGA_Sonic.jpg)."}
...
```{r map, out.width="50%", echo=FALSE, fig.cap="Locations of all eddy covariance towers in Europe."}
...
```{r Oe2, out.width="49%", fig.show='hold', echo=FALSE, fig.cap = "Left: MODIS tile with extracted region in black and location of eddy covariance tower in CH Oensingen as red cross. Right: Monthly NDVI vs $CO_{2}$ Flux of CH Oensingen Tower in 2014."}
...
```{r tower-data, out.width="49%", fig.show='hold', echo=FALSE, fig.cap = "Available data for all towers in data set from years 2000 to 2014. On the y- and x-axis with have the year, respectively the month of that year. Each plot denotes one of the total 34 towers in the data set."}
...
```{r tower-dist, out.width = "75%", echo=FALSE, fig.cap="Distribution of CO~2~ Flux by Tower."}
...
Note again, that the scale of the relevant eddy covariance tower footprint (a few hundred metres) is much smaller than the scale of the NDVI "scene" we're using here. This is *a priori* knowledge about the system that we should absolutely and always use when building any type of empirical model. It saves us from deviating into a world of spurious results, that, when being surprising enough, make it into high-impact journals nonetheless. Let us, for just now, ignore this and carry on using the entire NDVI scene for predicting local fluxes with a CNN.
...
```{r toke, echo=FALSE, out.width="150%", fig.cap="Visualization of adding a tokenization layer to a CNN."}
