...
Choose _Version Control_ if you would like to clone a remote repository from Git.
![](./figures/Git_RStudio_Create_Project_5_Version_Control.jpg)
You will need the URL of the remote repository so RStudio can clone it. The specified directory is where the remote repository will be cloned into. 
![](./figures/Git_RStudio_Create_Project_6_Version_Control.jpg)


4. **Using Git commands in RStudio**

When the project opens in Rstudio, you will be able to see the Git tab and the main commands you need to be able to benefit from Git:

- commit
- push
- pull

A _commit_ saves the current version of the file or project you are working on to the local repository. By continually adding commits you will get a timeline of your project as it develops. All commits are saved to the _local_ repository. 

To commit press the commit button or `Ctr + Alt + M`. 
![](./figures/Git_RStudio_Commit_1.jpg)
RStudio will open up a new window, where you can see the changes, the history and a box where you can add a commit message. The commit message is a way to later know what you changed in the file with that commit. Make sure you tick the box in the column _Staged_ next to the file you want to commit. 
![](./figures/Git_RStudio_Commit_2_Message.jpg)
If it worked you will see something like the following:
![](./figures/Git_RStudio_Commit_3.jpg)
Remember, this file is still only saved to the _local_ repository. To update it on the _remote_ repository too, you will need to _push_ it.

![](./figures/Git_RStudio_Push.jpg)
