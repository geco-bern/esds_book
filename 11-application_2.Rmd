...
## Application
### Problem Statement 
For this exercise we can use the reduced and cleaned dataset with half-hourly data from Chapter \@ref(ch-02). The task is to predict the *productivity of vegetation (GPP)*, using all the other features as predictor variables.
...
Recall the tutorial on *data wrangling*. We already performed some data-preprocessing there. We had replaced the missing value flag -9999 by NAs for the target variable **GPP_NT_VUT_REF**. 
...
```
```{r}
train_data_stat <- preProcess(train_data, method = c("center", "scale"))
...
summary(train_data_cs)
summary(test_data_cs)
```
...
When compiling the model take care to use appropriate optimiser, loss and evaluation metric. Use mean squared error as the loss, and mean absolute error as the evaluation metric 
...
Now that the model training is complete we can look at the history, plot the training history, and look at our trained weights. Pay attention to the trained weight matrices and try to recognise the transformation weights and the bias units.  
...
Use mean squared error as your loss and evaluation metric. Don't forget to use *early stopping* (callback_early_stopping()).
...
### Parameter Tuning
Here we will write a function that does the cross-validation for parameter tuning. The content of the cross-validation code remains the same as we have seen before. This is the same code written as a function that also takes the hyperparameter to be tuned as an argument.
Your task is to optimise the number of hidden units. 
First we will organise all the steps done until now to write the function. 
...
sprintf("Number of units for minimal average cross validation error: %i", num_units_best)
```

```{r}
## [1] "Number of units for minimal average cross validation error: 100"
```
