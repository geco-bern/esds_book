...
* _Confidence interval (CI)_ is a range of values that with a certain probability or degree of certainty the chosen parameter of the data variable will fall within.
* _Prediction interval_ is the estimated interval within which observations will occur, with a certain degree of confidence, based on the previously observed data.
...
This dataframe appears complicated, but we can transform the data into a useful spatial format. For that it needs to be converted to a raster. This is a key spatial data type and will be described in more detail in the section Raster below. 
...
df_nona <- ddf_ch_lae_ndvi %>% 
  drop_na()                                 # removes rows containing NAs
...
To see how well this data correlates with measurements from the fluxnet tower, we look at the relationship of gross primary productivity (GPP) vs. NDVI*PPFD (photosynthetic photon flux density). 
...
Above we mentioned that _SpatialPolygonsDataFrame_ contains both a list of _SpatialPolygons_ and a dataframe with information on each polygon. Here is how we can access the information in the dataframe. We will start by displaying the header of some of the 94 features for now.
...
By using the _SpatialPoints_, now in the correct coordinate reference system, and the data from _europe_shape_, we can bind them to our tower sites in the dataframe _df_sites_.
...
We first create a vector which is lists the country each towers is located in. We obtain this vector from the data frame _df_site_country_info_.  Then, we filter out only those countries with a tower in them and voilà, plot the countries and tower locations as a result.
...
This will give us a tidy dataframe with the site, latitude and longitude, temperatures for Chelsa and WorldClim at each tower location.
...
GPP <- ddf_allsites_nested_joined %>% 
  ...
  summarise(gpp_medianann = median(gpp_ann)) %>%
...
We can create a dataframe with all the extracted data from the tower sites to then make correlations.
...
Back to correlations, we can start by correlating the CHELSA temperatures with GPP. We do this by building a simple linear model using the function `lm()`.
...
Again, we can plot it. However, since landcover is categorical data we make a boxplot instead. A boxplot is a figure that gives a graphical representation of the variability or spread of the data, by providing information beyond the central tendency of the data. A boxplot will show ...
```{r warning=F}
