...
***
## Introduction
In this tutorial, we will put into practice and apply many of the models seen earlier in the course, in order to achieve a simple goal: efficiently extrapolate an environmental dataset. For this, we will use satellite data (specifically NDVI) as an accurate and "*cheap*" predictor of a specific environmental variable (C0~2~ - vegetation fluxes) that is measured scarcely by expensive physical devices (towers). Sit down and relax. This tutorial invites you to take a step back, and reflect on all the knowledge you have acquired on modelling. 
...
Using the eddy-covariance technique, it is possible to measure the CO~2~ gas exchange fluxes between the vegetation and the atmosphere at high temporal resolution and throughout the year. This is a key scientific measurement, for studying as an example the CO~2~ cycle. [Technical details on the measurement can be found here](https://en.wikipedia.org/wiki/File:Eddy_Covariance_IRGA_Sonic.jpg).

<center><img src<- "https://upload.wikimedia.org/wikipedia/commons/5/52/Eddy_Covariance_IRGA_Sonic.jpg" width<-"400px" class<-"center"><center>

Vegetation-atmosphere CO~2~ fluxes are driven by photosynthesis and respiration. The latter is responsible for ecosystem-level CO~2~ uptake, and the latter reverses the uptake flux. Below, as a reminder, is the equation of the reactions that happens during photosynthesis.

<center><img src <- "https://upload.wikimedia.org/wikipedia/commons/thumb/f/fd/Photosynthesis_equation.svg/640px-Photosynthesis_equation.svg.png" class<-"center"><center>
...
We will compare different approaches to find the function $f$ that best captures the relationship between NDVI (and other explanatory variables) to CO~2~ fluxes ($y_{CO2}$).
...
#### FLUXNET
We will once again be using the FLUXNET dataset, that you have gotten familiar with in the previous tutorials. We want to try to predict the variable $ y <- $ [`NEE_CUT_REF`](https://fluxnet.org/data/fluxnet2015-dataset/fullset-data-product/), which expresses the fluxes in gC $m^{-2} d^{-1}$, using the ndvi value of the corresponding region.

Here is the map showing the location of the 34 European towers available in our dataset:

<center><img src<-"./images/location_of_towers.png" style<-"width:50%;" class<-"center"><center>
...
Each MODIS product gives a tile with the desired information. From this tile a region around each tower is extracted. The following picture shows you an example of such a tile and the area around the CH Oerlikon tower.

<center><h4>Extracted Region (black square) around the Tower of CH Oerlikon (red cross) for the given Modis tile</h4></center>
<center><img src<-"./images/extracted_region_CH_Oe2.jpg" style<-"width:45%;" class<-"center"></center>
...
This might all seems a little abstract, so let's visualise it! The two animations below show (i) our monthly inputs (NDVI images) and (ii) the corresponding monthly outputs (CO~2~ flux plot) for the CH Oerlikon tower in the year 2014.

<center><h4> &emsp; &emsp; &emsp; &emsp; &emsp;Monthly NDVI vs $CO^{2}$ Flux of CH Oerlikon Tower in 2014</h4> </center> 
<center><img src<-"./images/ndvi_vs_CO2.gif" style<-"width:40%;"><center>
...
Before we start on the modelling part, it would be good to check how much data is available at each tower for the desired time (2000-2014). Take a look at the plots below, showing the monthly available data for each tower over the years. Blue shows data is available, while red shows data is unavailable.


<center><h3>Available Data by Tower (2000-2014)</h3></center>
<table><tr><td><img src<-'./images/data_available_1.png' style<-"width:90%;", ></td><td><img src<-'./images/data_available_2.png' style<-"width:90%;"></td></tr></table>
...
```
Let 's have a look at the distribution of CO~2~ flux by tower.

<center><h4>Distribution of CO~2~ Flux by Tower</h4></center>
<center><img src<-"./images/Distribution_per_Tower.png" style<-"width:40%" align<-"middle"></center>
...
We first build a linear model, which assumes a linear relationship between _label_ (output) and _feature_ (input) 
...
with $\beta_0,\beta_1 \in \mathbb{R}$
...
# train the model
# history_cnn = fit(model,x = ndvi_train_pr,y = y_train,batch_size = 128,
#              epochs = 200,shuffle = T,validation_data = list(ndvi_val_pr,y_val),callbacks = callbacks_cnn)
```
...
A visual representation of the *mixed model* is given in the following image:

<center><img src<-"./images/mixed_model.jpeg" style<-"width:70%;" class<-"center"></center>
...
pivot_longer(cols = 2:3,names_to = 'set',values_to = 'result')%>%
 ggplot(aes(x= set, y=result, fill=Model)) + 
   geom_bar( stat="identity", 
            position = "dodge",) +
   geom_text(aes(label=result), vjust=1.6, color="white",
...
g1 =   MSE_Tower%>%
        ...
        geom_bar( stat="identity", 
                position = "dodge",
                fill="steelblue")+
        geom_text(aes(label=mse_cnn), hjust=1.3, color="white",
...
g2 =   MSE_Tower%>%
        ...
        geom_bar( stat="identity", 
                position = "dodge",
                fill="steelblue")+
        geom_text(aes(label=mse_mixed), hjust=1.3, color="white",
                position = position_dodge(0.9), size=4)+
        ...
        annotate(
            geom = "text", x = 23, y = 3, 
            label = "CNN MSE", hjust = 0, vjust = 1, size = 3)+
        scale_color_manual(name = '',values = c(Total='red'),labels=c(paste('Total Test MSE: ',
                                                                            round(mixed_test_mse,3))))+
...
As you can see here, the mixed model - that also contains information on the tower location - improves the MSE for most towers in comparison to the CNN model.

By providing the period in the year for each image in the mixed model, the result should greatly improve. This is because seasonality plays an important role in vegetation metabolism.


<center><img src= "https://upload.wikimedia.org/wikipedia/commons/7/7e/I_want_you_for_U.S._Army_3b48465u_original.jpg" width="300" class="center"><center>

This is the exercise for this week.
