...
- The visualisation of cross-validation results can be visualised with the `plot(model_object)` or `ggplot(model_object)`, where `model_object` is the object returned by the function `train`.
...
<!-- Show the model performance (metrics and visualisation) on the validation sets of all cross validation folds combined. -->
