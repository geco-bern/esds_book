...
<ul>
  ...
  <li>Logistic regression is similar in model structure to linear regression. It is different in that it can capture non-linearities in data. 
    </li>
...
<thead>
	<tr><th></th><th scope=col>PPFD_IN</th><th scope=col>GPP_NT_VUT_REF</th></tr>
	<tr><th></th><th scope=col>&lt;dbl&gt;</th><th scope=col>&lt;dbl&gt;</th></tr>
</thead>
<tbody>
	<tr><th scope=row>1</th><td>-9999</td><td>-1.33329</td></tr>
	<tr><th scope=row>2</th><td>-9999</td><td>-2.77788</td></tr>
	<tr><th scope=row>3</th><td>-9999</td><td>-3.22186</td></tr>
	<tr><th scope=row>4</th><td>-9999</td><td>-3.24148</td></tr>
	<tr><th scope=row>5</th><td>-9999</td><td>-3.99006</td></tr>
	<tr><th scope=row>6</th><td>-9999</td><td>-4.29533</td></tr>
</tbody>
</table>
...
```
    ...
    F-statistic: 1.773e+05 on 1 and 143266 DF,  p-value: < 2.2e-16
...
<div style="background-color:rgba(0, 200, 0, 0.5);border-radius: 7px; padding:10px">
    <b>Checkpoint</b><br/>
Can you apply the gradient descent algorithm in order to find the minimum of $f(x) = x^2 +1$?  <br/>
    Use as starting point $x_0=10$, learning rate 0.01 and make 1000 iterations.   <br/>
    <i>Hint: Recall from Calculus I, that the derivative of $x^2$ is $2x$.<i>
</div>
# your code here
...
```
    ...
    Set session seed to 0 (disabled GPU, CPU parallelism)
...
![](./figures/animation_gd.mp4)

== HOW TO EMBED VIDEO? ==


__Left plot__: The gradient descent algorithm (blue trace).    __Right plot__: The fitting line for the respective step of gradient descent

What we actually observe is the fitted line to the data  (right plot) for each step of gradient descent (orange point
of left plot). As the gradient descent update step approaches the minimum of the loss function, the line fits the data better, i.e it minimizes $\sum_{i = 1}^{n} (\hat{y}_i-y_i)^2$. 
...
Next let's visualize the best fit inferred by gradient descent, as compared to the exact solution, in the conext of the data we're trying to fit.
...
df_train %>%
    ...
    geom_abline(aes(intercept = b, slope = w,color='GD Solution',linetype='GD Solution'),lwd =2) +
    ##plot exact line
...
model = load_model_hdf5(save_path)
...
<thead>
	<tr><th scope=col>Model</th><th scope=col>Train_MSE</th><th scope=col>Test_MSE</th></tr>
	<tr><th scope=col>&lt;chr&gt;</th><th scope=col>&lt;dbl&gt;</th><th scope=col>&lt;dbl&gt;</th></tr>
</thead>
<tbody>
	<tr><td>Null  </td><td>77.73053</td><td>77.37176</td></tr>
	<tr><td>Linear</td><td>34.77979</td><td>34.42075</td></tr>
</tbody>
</table>
...
So far in this tutorial, we have studied gradient descent in the context of linear regression. Such a linear model is given by the following formula: 
...
Let's consider a hypothetical binary classification example. That is, assume $y$ either takes the value $0$ (no cat) or $1$ (cat). We are interested in modeling the relation between $P(Y=1|x)$ and $x = (x_1,x_2,...,x_n)$.

The aforementionted relation can be defined as 

$$
P(Y=1|x) = \sigma(w_1x_1 +...+ w_nx_n + b)
$$
where $$\sigma(t) = \frac{1}{1+e^{-t}}$$

Here is what a logistic function looks like:
...
```
    ...
    Proportion of examples with label 0:  48.57143 %
    Proportion of examples with label 1:  51.42857 %
