...
cat('\nFinal solution: w =', w_est, ' b = ', b_est, "\n")
```

```{r eval = F}
# Load pre-calculated model to skip computation
...
![Animated gradient descent: __Left plot__: The gradient descent algorithm (red trace). __Right plot__: The fitting line for the respective step of gradient descent.](https://miro.medium.com/proxy/0*D7zG46WrdKx54pbU.gif)

What we actually observe is the fitted line to the data (right plot) for each step of gradient descent (red point of left plot). As the gradient descent update step approaches the minimum of the loss function, the line fits the data better, i.e it minimizes $\sum_{i = 1}^{n} (\hat{y}_i-y_i)^2$. 
...
cat("\n Loss on test data: 34.42075 \n") # Number added by hand for computational reasons
...
The aforementioned relation can be defined as $P(Y=1|x) = \sigma(w_1x_1 +...+ w_nx_n + b)$, where $\sigma(t) = \frac{1}{1+e^{-t}}$
