...
<div style="border: 2px black solid; border-radius: 7px; padding:10px">
    <b>Important points from the lecture</b><br/>
    <br>
    <ul>
<li>For binary classification problems, the output node often uses a <i>sigmoid activation function. </i></li>
<li>A <i>classification threshold</i> is applied to the output of the sigmoid activation function to determine which label is predicted.</li>
<li><i>Accuracy</i> is a popular measure of model performance, but it is not useful for class-imbalanced data.</li>
<li>The area under the <i>ROC curve </i> is a measure of model performance that considers the full range of possible classification thresholds.</li>
...
In this toy dataset, each example has two real-valued features on the unit interval (i.e., these values are between 0 and 1). Each example has one of two labels, <font color =green> green </font> or <font color =blue> blue </font>. The dataset is constructed such that those examples _within_ a specified radius of the point (0.5, 0.5) are assigned a <font color =green> green </font> label and those _outside_ this radius are assigned a <font color =blue> blue </font> label. 
...
Let's visualize our data to get a feel for the classification task at hand. Let's also visualize the true decision boundary used to delineate the <font color =green> green </font> from the <font color =blue> blue </font> labels, and print the number of examples per label. 

_Notice_ that there are more examples labeled <font color =blue> blue </font> than there are examples labeled <font color =green> green </font>. 
...
```
    ...
    Proportion of data in class green:  0.3982 
...
```
    ...
    Training set 
    Proportion of data in class blue:  0.6017998 
    Proportion of data in class green:  0.3982002 
    
    Test set 
    Proportion of data in class blue:  0.6018009 
    Proportion of data in class green:  0.3981991 
...
<div style="border: 2px black solid; border-radius: 7px; padding:10px">
<b>Neural Networks: A closer look</b><br/>
    <br>
...
```
    ...
    Accuracy on held-out test data:  0.602 
...
In this toy dataset, we'll call the <font color=blue>blue</font> examples <font color=blue>_'positive'_</font> and the <font color=green>green</font> examples <font color=green>_'negative'_</font>. This terminology makes more sense in more realistic classification problems, such as delineating spam email from non-spam.
...
Now we see that the model "<font color=blue>always blue</font>" does a great job at correctly predicting positive examples (_'recall'_), but many of its positive predictions are incorrect (_'precision'_).
...
```{r}
c(model,history) %<-% build_and_train_model(num_layers = 1,num_units_per_layer =  2,print_model_summary = T)
cat('Final training accuracy: ', round(history$metrics$acc[ne],3),'\n')
```
...
c(true_positives, true_negatives,false_positives, false_negatives) %<-% confusion(df_test$y, pred, threshold,verbose=F)
c(precision, recall) %<-% precision_and_recall(true_positives, true_negatives,false_positives, false_negatives)
f = f_score(precision,recall)
```

    Accuracy on held-out test data:  0.75 
    Precision:  0.8490566 
    Recall:  0.7107232 
    F-score:  0.7737557
...
```{r}
c(model,history) %<-% build_and_train_model(num_layers = 4, num_units_per_layer =  8,print_model_summary = T)
cat('Final training accuracy: ', round(history$metrics$acc[ne],3),'\n')
...
```
    ...
    Non-trainable params: 0
    ________________________________________________________________________________
    Final training accuracy:  0.935 
    Accuracy on held-out test data:  0.933 
    Precision:  0.9370915 
    Recall:  0.9534497 
    F-score:  0.9451998
...
```{r}
df = data.frame(x = thresholds,metrics = c(precision, recall),type = rep(c('precision','recall'),each=num_thresholds))

df %>%
    ggplot(aes(x=x,y=metrics,color = type))+
    geom_line(lwd=1.5,lty=2)+
    scale_color_manual(name = '',labels = c('Precision','Recall'),values = c('Red','Blue'))+
...
<div style="background-color:rgba(0, 200, 0, 0.5);border-radius: 7px; padding:10px">
<b>Checkpoint</b><br/>
1. Can you think of a real world examples where it's neccessary to have a high precision model (and potentially low recall)? <br>
...
roc_object = roc(df_test$y~ c(pred),plot=TRUE,print.auc=TRUE,col="blue",lwd =4,legacy.axes=TRUE,main="ROC Curve")
```
...
for (i in 1:num_models){
    ...
    for (k in 1:num_initializations){
        ...
        for (j in 1:num_thresholds){
            ...
            c(tp[j,k,i], tn[j,k,i],fp[j,k,i], fn[j,k,i]) %<-% confusion(df_test$y, pred, thresholds[j],verbose = F)
            c(precision[j,k,i], recall[j,k,i]) %<-% precision_and_recall(tp[j,k,i], tn[j,k,i],fp[j,k,i], fn[j,k,i],verbose = F)
...
```
    ...
    You look nice today. Model configuration 1 initialization 1
    
    
    You look nice today. Model configuration 1 initialization 2
    
    
    You look nice today. Model configuration 1 initialization 3
    
    
    You look nice today. Model configuration 1 initialization 4
    
    
    You look nice today. Model configuration 1 initialization 5
    
    
    You look nice today. Model configuration 1 initialization 6
    
    
    You look nice today. Model configuration 1 initialization 7
    
    
    You look nice today. Model configuration 1 initialization 8
    
    
    You look nice today. Model configuration 1 initialization 9
    
    
    You look nice today. Model configuration 1 initialization 10
    
    
    You look nice today. Model configuration 2 initialization 1
    
    
    You look nice today. Model configuration 2 initialization 2
    
    
    You look nice today. Model configuration 2 initialization 3
    
    
    You look nice today. Model configuration 2 initialization 4
    
    
    You look nice today. Model configuration 2 initialization 5
    
    
    You look nice today. Model configuration 2 initialization 6
    
    
    You look nice today. Model configuration 2 initialization 7
    
    
    You look nice today. Model configuration 2 initialization 8
    
    
    You look nice today. Model configuration 2 initialization 9
    
    
    You look nice today. Model configuration 2 initialization 10
    
    
    You look nice today. Model configuration 3 initialization 1
    
    
    You look nice today. Model configuration 3 initialization 2
    
    
    You look nice today. Model configuration 3 initialization 3
    
    
    You look nice today. Model configuration 3 initialization 4
    
    
    You look nice today. Model configuration 3 initialization 5
    
    
    You look nice today. Model configuration 3 initialization 6
    
    
    You look nice today. Model configuration 3 initialization 7
    
    
    You look nice today. Model configuration 3 initialization 8
    
    
    You look nice today. Model configuration 3 initialization 9
    
    
    You look nice today. Model configuration 3 initialization 10
    
    
    You look nice today. Model configuration 4 initialization 1
    
    
    You look nice today. Model configuration 4 initialization 2
    
    
    You look nice today. Model configuration 4 initialization 3
    
    
    You look nice today. Model configuration 4 initialization 4
    
    
    You look nice today. Model configuration 4 initialization 5
    
    
    You look nice today. Model configuration 4 initialization 6
    
    
    You look nice today. Model configuration 4 initialization 7
    
    
    You look nice today. Model configuration 4 initialization 8
    
    
    You look nice today. Model configuration 4 initialization 9
    
    
    You look nice today. Model configuration 4 initialization 10
...
```
    ...
    Auc for model  1  is  0.677 
    Auc for model  2  is  0.611 
    Auc for model  3  is  0.929 
    Auc for model  4  is  0.981 
...
#Build a data frame
df = data.frame(avg_fpr = c(average_fpr[,1],average_fpr[,2],average_fpr[,3],average_fpr[,4]),
               avg_recall = c(average_recall[,1],average_recall[,2],average_recall[,3],average_recall[,4]),
               Model= rep(model_name,each = num_thresholds))

#Plot the results
df %>% ggplot(aes(x = avg_fpr,y = avg_recall,color = Model))+
      geom_line(lwd=1,lty=2)+
      geom_line(data = data.frame(x=x),mapping = aes(x=x,y=x),inherit.aes = F,alpha=0.7)+
      labs(title='Area Under the Curve ROC',x = 'False Positive Rate' ,y = 'Recall')+
      xlim(0,1)+
...
Again you can see the most complex model (<font color=purple>8 layers, each with 8 nodes</font>) clearly outperforms the less complex ones.
...
```
    ...
    Proportion of data in class green:  0.8183 
...
```{r}
#Build and train the model
c(model,history) %<-% build_and_train_model(data = df_train_new , num_layers = 8, num_units_per_layer =  8, print_model_summary = T)
...
#Print precision,recall, and F-score
c(true_positives, true_negatives,false_positives, false_negatives) %<-% confusion(df_test_new$y, pred, threshold,verbose = F)
c(precision, recall) %<-% precision_and_recall(true_positives, true_negatives,false_positives, false_negatives)
f = f_score(precision,recall)
```
...
for (i in 1:num_thresholds){
    ...
    c(tp[i], tn[i],fp[i], fn[i]) %<-% confusion(df_test_new$y, pred, thresholds[i],verbose = F)
    c(precision[i], recall[i]) %<-% precision_and_recall(tp[i], tn[i],fp[i], fn[i],verbose = F)
    }
