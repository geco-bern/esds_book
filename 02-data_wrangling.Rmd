...
```{r, message=FALSE}
library(tidyverse)
hhdf <- read_csv("./data/fluxnet_hourly/data_HH/FLX_CH-Lae_FLUXNET2015_FULLSET_HH_2004-2014_1-3.csv")
hhdf
```
...
```{r}
save(hhdf, file = "./data/fluxnet_hourly/data_HH/FLX_CH-Lae_FLUXNET2015_FULLSET_HH_2004-2014_1-3_CLEAN.RData")
```
...
```{r}
vec_files <- list.files("./data/fluxnet_daily/data_DD", pattern = "_FLUXNET2015_FULLSET_DD_", full.names = TRUE)
print(vec_files[1:5])
```
...
```{r}
df_sites <- read_csv("./data/fluxnet_daily/data_DD/fluxnet_site_info_all.csv")
head(df_sites)
```
...
```{r}
save(ddf_allsites_nested_joined, file = "ddf_allsites_nested_joined.RData")
```
...
save(ddf_ch_lae, file = "ddf_ch_lae.RData")
```
