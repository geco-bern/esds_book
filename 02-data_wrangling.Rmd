...
[**tidyverse**] [@R-tidyverse] (https://www.tidyverse.org/) is a collection of packages and `library(tidyverse)` loads them all. For this tutorial we use in particular:
...
Aggregating in data science refers to the process of bringing together data to form a more summarised version of the different data. This can be data from multiple sources or just a compilation of the data already present. In R this can be done within a data frame or between data frames. Here, we will group our data by a specific variable (date) and summarise the variable `GPP_NT_VUT_REF`.
...
We observe that there is a tendency for the `f_measured` to be higher (i.e. green and dark blue) as PPFD increases, there is no equivalent trend for GPP. The colours showing the data quality or `f_measured` are mixed across the range of the vales for GPP. The high GPP values, we previously found to be associated with low fractions of underlying measured data in the time series plot, are not explained by simultaneously high PPFD. In the plot, we see a core cluster of points in the centre of the positive trend with fewer points outside this denser area. These outlying points are lighter and do not fit the linear relationship as well.

Despite scattered data points, there is a positive linear relationship between GPP and PPFD. To find the best fit of this linear relationship, meaning the straight line that best fits our data points, we will move on to modelling using a univariate linear regression. The section below, will serve as a brief introduction, since later chapters will go into more detail on modelling regressions.
...
In the figure, we can observe a much higher density of points between PPFD values 0 and 200 and between GPP values of -2 to 5. The density gets weaker as as PPFD increases but GPP stays within the same values. We can also observe a second cluster, albeit at a lower density. This starts at a PPFD value of around 200 and at a GPP value over 5. This second cluster has a steeper slope creating an interesting "separation" of points, where the increase in GPP with increasing PPFD is steeper for some than for others. 
  Again, we can use our *a priori* understanding of the system to formulate hypotheses and test them by finding the appropriate visualisation type. Here, one hypothesis could be that the slope is steeper in some months than in others. This is actually more than just a vague guess. **XXX BENI: how come this is more than a vague guess? Explaining how the relationship between GPP and PPFD gets you to this hypothesis would help here.XXX**
...
We see that overall, there is 3.7% missing data. Specifically, `PPFD_IN` is often missing. XXX BENI: What is the consequence of PPFD_IN often being missing? Please expand on this a bit. XXX
...
The paths each start with the subdirectory where they are located (`"./data/"`), then `"FLX_"`, followed by the site name (the first three entries of the table containing data from all sites are for the site `"BE-Bra"`), and then some more specifications, including the years that respective files' data cover. What is the most effective way to extract the site name from all these strings? The **stringr** package offers a set of very handy tools to work with strings and regular expressions, see this [cheat sheet](https://evoldyn.gitlab.io/evomics-2018/ref-sheets/R_strings.pdf) to see how to find for specific patterns in a string and many other useful tools for manipulating strings or go [here](https://r4ds.had.co.nz/strings.html) for more general introduction with examples. 
